...
- **Warcraft III 전용 모드** — 워크3 창이 활성화된 경우에만 키 변환 동작 (다른 프로그램에 영향 없음)
- **토글 단축키** — 설정한 핫키(기본 F5)로 리매핑 활성/비활성 전환
- **시스템 트레이** — 최소화 시 트레이 아이콘으로 상주, 우클릭 메뉴 제공
- **설정 자동 저장** — JSON 형식으로 `%APPDATA%\ItemCast\config.json`에 저장
- **단일 인스턴스** — 중복 실행 시 기존 창을 활성화
- **Reforged 호환** — 클래식/리포지드 모두 감지
...
4. **Warcraft III 전용** 체크 시 워크3 창에서만 키가 변환됩니다.
5. 토글 키 버튼을 클릭하여 활성/비활성 전환 단축키를 변경할 수 있습니다.
